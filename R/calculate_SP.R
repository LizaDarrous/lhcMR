...
#'
#' @examples
calculate_SP <- function(input.df,trait.names,log.file,run_ldsc=TRUE,run_MR=TRUE,hm3,ld,run_SingleStep=FALSE,
                         run_TwoStep=TRUE,SP_single=50,SP_pair=100,partition,SNP_filter=10){
  ...
  ## option to get piX, iX (and total_h2) in a separate step
  if(run_TwoStep==TRUE){
  ...
  if(run_TwoStep==TRUE){
    ...
    colnames(sp_mat)=c("sp_h2X","sp_h2Y","sp_tX","sp_tY","sp_alp","sp_bet","sp_iXY")
    sp_mat = cbind("SP"=c(1:nrow(sp_mat)),sp_mat)
    write.csv(sp_mat,"StartingPoints.csv", row.names=F) #SP_pair

  }
  if(run_SingleStep==TRUE){
    sp_piX = pi_X+runif(SP_pair,-1e-4,1e-4) #rep(pi_X,SP_pair)
    sp_piY = pi_Y+runif(SP_pair,-1e-4,1e-4) #rep(pi_Y,SP_pair)
    sp_tX = runif(SP_pair,0,0.5)
    sp_tY = runif(SP_pair,-0.5,0.5)
    sp_h2X = h2_x-(sp_tX^2)
    sp_h2Y = h2_y-(sp_tY^2)
    ...
    sp_mat=matrix(unlist(para), ncol=9, byrow = FALSE)
    colnames(sp_mat)=c("sp_piX","sp_piY","sp_h2X","sp_h2Y","sp_tX","sp_tY","sp_alp","sp_bet","sp_iXY")
    write.csv(sp_mat,"StartingPoints.csv", row.names=F) #SP_pair
  }

  return(list("iX"=i_X,"iY"=i_Y,"piX"=pi_X,"piY"=pi_Y,"input.df_filtered"=input.df_filtered,"sp_mat"=sp_mat))
}
